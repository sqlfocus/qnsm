...
static TmEcode TmThreadWaitOnPPTInit(void)
{
    ThreadVars *tv = NULL;
    uint16_t ppt_num = 0;
    uint64_t slept = 0;
    ...
    {
        if (EN_QNSM_DETECT == app_type[lcore_id])
        {
            n_ppt_tm++;
        }
    }
...
again:
    ...
    SCLogNotice("all %"PRIu16" packet processing threads, engine started.", ppt_num);
    return TM_ECODE_OK;
}

void qnsm_idps_wait(void)
{        
    if (TmThreadWaitOnPPTInit() == TM_ECODE_FAILED) {
        SCLogNotice("Wait PPT init failed.");
        exit(EXIT_FAILURE);
    }
